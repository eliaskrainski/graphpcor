...
graphical approach to model correlations proposed in @dtg2024model.
...
We use the DTG to represent variables and the
directed edges (arrows) to represent conditional distributions.
In addition, the nodes of the DTG are classified as
parent or children.
The nodes classified as children are leafs of the DTG, 
...
g1 <- dtg(p1 ~ c1 + c2 + c3)
g2 <- dtg(p1 ~ p2 + c1 + c2, p2 ~ c3 + c4)
g3a <- dtg(p1 ~ p2 + p3 + c1 + c2, p2 ~ c3 + c4, p3 ~ c5 + c6)
g3b <- dtg(p1 ~ p2 + c1 + c2, p2 ~ p3 + c3 + c4, p3 ~ c5 + c6)
...
The proposed model is based on a DTG and induces a
...
We now define a set of rules to propose a model from a DTG.
...
The arrows in the DTG represent each of these conditional distributions.
...
g4a <- dtg(p1 ~ p2 + p3 + c1 + c2, 
           p2 ~ p4 + c3 + c4, 
           p3 ~ p5 + c5 + c6, 
           p4 ~ c7 + c8, p5 ~ c9 + c10)
g4b <- dtg(p1 ~ p2 + p3 + c1 + c2, 
