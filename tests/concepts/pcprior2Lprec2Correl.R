library(numDeriv)

nrepl <- 5000

theta2L <- function(theta) {
### imput theta output L such Q = LL'
    m <- length(theta)
    n <- (1 + sqrt(1+8*m))/2
    stopifnot((n==floor(n)) & (n==ceiling(n)))
    stopifnot(n>0)
    L <- diag(n)
    L[lower.tri(L)] <- theta
    return(L)
}

theta1 <- c(0, -1, -2)
theta2L(theta1)

theta2C <- function(theta) {
### imput theta output C
    V <- chol2inv(t(theta2L(theta)))
    s <- sqrt(diag(V))
    t(V/s)/s
}

theta2C(c(0,0,0))
theta2C(c(1,1,1))
theta2C(theta1)

theta2KLD <- function(theta, C0) {
### imput theta ouptut KLD
### KLD = 0.5( tr(C0^{-1}C1) -p + ... - log(|C1|) + log(|C0|) )
    C1 <- theta2C(theta)
    p <- nrow(C1)
    l1 <- chol(C1)
    hld1 <- sum(log(diag(l1)))
    if(missing(C0)) {
        warning("Missing base model!")
        C0 <- diag(rep(1, p), p, p)
    } 
    l0 <- chol(C0)
    hld0 <- sum(log(diag(l0)))
    tr <- sum(diag(chol2inv(l0) %*% C1))
    0.5*(tr -p) + hld0 - hld1
}

theta0 <- c(-3,-3,-3)/30
cc0 <- theta2C(theta0)
cc0

theta2KLD(c(0,0,0))
theta2KLD(theta0, cc0)
theta2KLD(theta1, cc0)

theta2H <- function(theta) {
### imput theta output H, H^0.5, svd(H)
    if(missing(theta)) stop('Please provide "theta"!')
    C0 <- theta2C(theta)
    H <- hessian(function(x) theta2KLD(x, C0),
                 theta)
    sv <- svd(H)
    if(any(sv$d<sqrt(.Machine$double.eps) * abs(sv$d[1])))
        warning("'H' is numerically not positive definite")
    h.5 <- t(sv$v %*% (t(sv$u) * sqrt(sv$d)))
    hneg.5 <- t(sv$v %*% (t(sv$u) / sqrt(sv$d)))
    stopifnot(all.equal(H, tcrossprod(h.5)))
    list(H = H,
         h.5 = h.5,
         hneg.5 = hneg.5,
         svdH = sv)
}

h0 <- theta2H(theta0)
h0

solve(h0$H)
cov(matrix(rnorm(1e3*3), ncol = 3) %*% h0$hneg.5)

rphi2x <- function(rphi) {
### to convert from \{r, \phi_1, ..., \phi_{m-1} \} into x_i \in \Re
### see https://en.wikipedia.org/wiki/N-sphere
    co <- cos(rphi[-1])
    si <- cumprod(sin(rphi[-1]))
    x <- rphi[1] * c(co, 1) * c(1, si)
    return(x)
}

rtheta <- function(n, lambda=1, R, mu0) {
    m <- n*(n-1)/2
    r <- rexp(1, lambda) ## radial coordinate
    if(m>1) {
        phi <- numeric(m-1)
        if(m>2)
            phi[1:(m-2)] <- runif(m-2, 0, pi)  ## m-2 angles
        phi[m-1] <- runif(1, 0, 2*pi)  ## last angle
        
    } else {
        phi <- NULL
    }
    drop((rphi2x(c(r, phi)) %*% R) + mu0)
}

## sample correlation matrices around C0 with lambda = 1
s1 <- t(replicate(nrepl, theta2C(rtheta(3, 1, h0$hneg.5, theta0))[lower.tri(diag(3))]))

## sample correlation matrices around C0 with lambda = 100
s10 <- t(replicate(nrepl, theta2C(rtheta(3, 10, h0$h.5, theta0))[lower.tri(diag(3))]))

### compare the samples with C0
cc0[lower.tri(diag(3))]

apply(s1, 2, summary)
apply(s50, 2, summary)

x2rphi <- function(x) {
### to convert from x_i \in \Re into \{r, \phi_1, ..., \phi_{m-1} \}
### see https://en.wikipedia.org/wiki/N-sphere
### NOTE: from x to phi it may give phi[m-1]<0, if so add 2*pi
    m <- length(x)
    if(m>1) {
        phi <- numeric(m-1)
        phi[m-1] <- atan2(x[m], x[m-1])
        if(phi[m-1]<0)
            phi[m-1] <- phi[m-1] + 2*pi
        r2 <- x[m]^2 + x[m-1]^2
        if(m>2) {
            for(i in (m-2):1) {
                phi[i] <- atan2(sqrt(r2), x[i])
                r2 <- r2 + x[i]^2
            }
        }
    } else {
        phi <- NULL
    }
    return(c(sqrt(r2), phi))
}

c1c2 <- t(replicate(nrepl, {
    rphi <- c(rexp(1,1), runif(6-2, 0, pi), runif(1, 0, 2*pi))
    x <- rphi2x(rphi)
    c1 <- all.equal(x, rphi2x(rphi))
    c2 <- all.equal(rphi, x2rphi(x))
    c(is.logical(c1) && c1,
      is.logical(c2) && c2)
}))

table(c1c2[, 1], c1c2[, 2])

dtheta <- function(theta, lambda, theta0, H) {
    ## log(\lambda)-r\lamda -log(2)-(m-1)log(\pi) +log(|J1|) +log(|J2|)
    ## log(|J1|) = {m-1}*log(r) + \sum_{i=1:m-1} (m-i-1)*log(sin(\phi_i))
    ## log(|J2|) is numerically evaluated as log(abs(det(H)))
    stopifnot(length(theta)==length(theta0))
    m <- length(theta)
    ld2 <- 2*sum(log(abs(H$svd$d))) ## log(abs(det(H)))
    x <- drop(H$hneg.5 %*%theta) - theta0
    rphi <- x2rphi(x)
    stopifnot(all(rphi>0))
    ld1 <- (m-1) * log(rphi[1])
    if(m>2) {
        ii <- 1:(m-2) ### do not include the last one!
        ld1 <- ld1 + sum(log(sin(rphi[ii+1])) * (m-ii-1))
    }
    ld <- log(lambda) -rphi[1]*lambda -log(2) - (m-1)*log(pi)
    return(ld + ld1 + ld2)
}

par(mfrow = c(2, 4), mar = c(4,4,2,1), mgp = c(3,1,0), bty = 'n')
for(l in c(0.5, 1, 2, 10)) {
    theta.samples <- t(replicate(nrepl, rtheta(3, lambda = l, h0$h.5, theta0)))
    thetas.d <- apply(theta.samples, 1, dtheta, lambda = l, theta0 = theta0, H=h0)
    plot(rowSums(theta.samples^2), thetas.d)
    title(main = substitute(lambda == l, list(l = l)))
    plot(apply(theta.samples, 1, function(x) tcrossprod(x %*% h0$H, x)),
         thetas.d, xlab = expression((x-theta[0])~H(theta[0])~(x-theta[0])))
    title(main = substitute(lambda == l, list(l = l))) 
}

