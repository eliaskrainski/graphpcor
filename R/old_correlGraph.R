...
#'  \item JD same but as a function
#'  \item STR model structure
#'  \item Pmat precision matrix
...
#' @param SD1 standard deviation for model 1
...
#' @param lambda parameter of the exponential prior on the distance
...
GraphPlot <- function(S, base=0, fontsize=c(14, 14), width=c(0.75, 0.75), height=c(0.5,0.5)){
  ...
  nAttrs$color <- c(sapply(nod[grep("p", nod)], function(x) x="red"), sapply(nod[grep("c", nod)], function(x) x="limegreen"))
  nAttrs$fillcolor <- c(sapply(nod[grep("p", nod)], function(x) x="lightsalmon"), sapply(nod[grep("c", nod)], function(x) x="lightgreen"))
